...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    static int optimalPercentOfMutation = 0;
    static int[] numOfIteration = new int[101];
            ...
            finding:
            {
                do {
                    ...
                    for (int i = 0; i < rndSolvesRes.length; i++) {
                        if (rndSolvesRes[i] == y) {
                            for(int m = 0; m < rndSolves[i].length; m++){
                                result[m] = rndSolves[i][m];
                            }
                            break finding;
                        }
                    }
                    ...
                    mutation(y);
                    iterNum++;
                    numOfIteration[optimalPercentOfMutation] = iterNum;
                } while (iterNum < 10000);

            }

            if (iterNum >= 10000) {
                //System.out.println("Solving was not found.");
            }
            if(optimalPercentOfMutation < 101){
                optimalPercentOfMutation++;
            }
        }while(optimalPercentOfMutation != 100);
        int min = 10000;
        take :for(int m = 0; m < numOfIteration.length; m++){
            if(min > numOfIteration[m] && numOfIteration[m] != 0){
                min = numOfIteration[m];
                optimalPercentOfMutation = m;
            }
            if(min == 1){
                break take;
            }
        }
        tvResult.setText("x1 = " + result[0] + ", x2 = " + result[1] + ", x3 = " + result[2] + ", x4 = " + result[3] + "\n"
                +"Оптимальний відсоток мутації: " + optimalPercentOfMutation + "%");
    }
